...
-  **Supported Attacks**: As of now, DeepProv supports the following attacks : <br  />
	- MNIST: Fast Gradient Sign Method (FGSM), Projected Gradient Descent (PGD) Auto PGD with DLR loss function (APGD-DLR)  and Square ,SPSA is not included because it is computationally expensive and requires significant human intervention to use efficiently, such as manually saving and loading the generated attack samples throughout the experiments <br  />
